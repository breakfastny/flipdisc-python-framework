...
class OpticalFlow(object):
    ...
    def update_settings(
        self, trackrate=None, max_features=None, min_threshold=None, reverse=None
    ):
    ...
    def update(self, frame, emitter):
        ...
        if self._prev_frame is not None:
            if self._features is None or self._frame_count % self._trackrate == 0:
                self._features = cv2.goodFeaturesToTrack(
                    self._prev_frame, self._max_features, 0.005, 3.0
                )
        ...
        for indx, (feat, status) in enumerate(zip(features, feat_status)):
            ...
            origin = self._prev_features[indx][0][
                ::-1
            ]  # XXX is there a guarantee that this indx exists?
